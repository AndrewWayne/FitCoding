...
- **Mac camera permission.** TCC dialog must appear on first launch; the Tauri webview triggers `getUserMedia()`, which routes through the OS prompt. README must call this out explicitly. Implementation note: because `fitcoding` is an unbundled CLI binary, macOS attributes camera requests to the parent terminal (Warp, iTerm, Claude Code's shell) via the TCC responsibility chain — that terminal has no `NSCameraUsageDescription`, so TCC hard-crashes the process before any prompt can appear. To work around this, the `Launch` subcommand calls `responsibility_spawnattrs_setdisclaim` and re-spawns itself once on macOS, making the child its own responsible process so TCC reads the embedded `NSCameraUsageDescription` from the binary's `__TEXT,__info_plist` section. Re-spawn is gated by the `FITCODING_TCC_DISCLAIMED` env sentinel to prevent loops, and is macOS-only.
